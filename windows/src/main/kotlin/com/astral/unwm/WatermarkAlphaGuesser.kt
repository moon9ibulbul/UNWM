...
object WatermarkAlphaGuesser {
    fun guessAlpha(
        ...
        val baseBitmap = if (base.type != BufferedImage.TYPE_INT_ARGB) {
            val newImg = BufferedImage(base.width, base.height, BufferedImage.TYPE_INT_ARGB)
        ...
        val basePixelsFull = ImageUtils.getPixels(baseBitmap)
        val watermarkPixelsFull = ImageUtils.getPixels(watermarkBitmap)

        // We need to extract the overlap region for easier processing,
        // or just calculate indices carefully.
        // The original code used getPixels with stride/offset.
        // We have full arrays.
        ...
        for (y in 0 until overlapHeight) {
            val bY = baseStartY + y
            val wY = wmStartY + y
            val bRow = bY * baseBitmap.width
            val wRow = wY * watermarkBitmap.width
            val destRow = y * overlapWidth
            System.arraycopy(basePixelsFull, bRow + baseStartX, basePixels, destRow, overlapWidth)
            System.arraycopy(watermarkPixelsFull, wRow + wmStartX, watermarkPixels, destRow, overlapWidth)
        }
