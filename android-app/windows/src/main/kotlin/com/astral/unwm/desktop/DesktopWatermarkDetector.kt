...
data class WatermarkDetection(val offsetX: Float, val offsetY: Float, val score: Float)

object DesktopWatermarkDetector {
    fun detect(
        ...
        if (base.width < watermark.width || base.height < watermark.height) return emptyList()
        val baseMat = base.toMat()
        val watermarkMat = watermark.toMat()
        val baseGray = Mat()
        ...
        return try {
            ...
            if (nonZero.empty()) return emptyList()
            ...
            resultGray = Mat()
            Imgproc.matchTemplate(baseGray, watermarkGrayRoi, resultGray, Imgproc.TM_CCORR_NORMED, watermarkMaskRoi)
            ...
            resultEdges = Mat()
            Imgproc.matchTemplate(baseEdges, watermarkEdges, resultEdges, Imgproc.TM_CCORR_NORMED)
            ...
            val maxResultX = max(0.0, (combinedResult.cols() - 1).toDouble())
            val maxResultY = max(0.0, (combinedResult.rows() - 1).toDouble())
